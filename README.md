...
`tests/run_tests.sh` runs 12 programs written in the language subset common to
...
```
01_arithmetic  02_strings   03_tables   04_closures   05_metatables  06_coroutines
07_errors      08_control   09_algorithms 10_const_5_4 11_utf8        12_strpack
```

Result: **12 / 12 identical.** Cases `10_const_5_4`, `11_utf8` and `12_strpack`
exercise the ported `<const>` attribute, `utf8` library and
`string.pack`/`unpack`/`packsize` — all pass only because LuaJIT now matches
Lua 5.4.8 for those features.
